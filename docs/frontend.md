...
- **Avatar:** The Cole logo — a green graduation-cap chat-bubble icon (`cole-logo.png` via `ColeAvatar` component). Portrait aspect ratio (≈0.77:1), sized by height with `w-auto` to avoid stretching. Appears in the sidebar header (h-8), welcome state (h-14), message labels (h-5), and streaming indicator. A separate square version (`cole-favicon.png`, 512x512 with transparent padding) is used as the browser tab favicon to avoid stretching.
...
- **Default screen** (`showDefaultScreen` prop, default `true`): when query is empty, shows a sectioned view — "My Schools" (from `profile.savedSchools`, rendered in the user's profile-defined order so the top pick in Profile → Saved Schools is the top row here) followed by "All Schools" (remaining schools, alphabetized, filling the remaining budget up to `MAX_RESULTS - validSavedSchools.length` items). Section headers are non-interactive `div` labels matching `MajorCombobox` styling. Saved schools that no longer exist in `collegeOptions` are filtered out. A static "Not Specified" row (value: null) sits at the very top.
...
- **Alphabetical ordering:** the "All Schools" section and all filtered/search lists are sorted via `localeCompare` with `{ sensitivity: 'base' }` on `formatSchoolName(c)`, so display order is case- and diacritic-insensitive. The "My Schools" section is the exception: it preserves `profile.savedSchools` order (add order + any drag-reorders in the Profile tab) instead of being alphabetized.
