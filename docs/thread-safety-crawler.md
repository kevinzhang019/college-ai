...
- **`PlaywrightPool`** — thread-local browser instances (`threading.local()`) because Playwright sync API is not thread-safe. Browsers rotate after `PLAYWRIGHT_POOL_ROTATE_AFTER=50` uses for fresh fingerprints. Supports both Chromium and Camoufox. Each slot tracks `_owner_tid` (owning thread ID) for orphan detection. `shutdown()` sets `_started = False` inside `_all_locals_lock` to create a happens-before with any thread in `acquire()` — this prevents new browser creation after shutdown begins. All three branches in `acquire()` (rotation, creation, **and reuse**) check `_started` under `_all_locals_lock` before returning a browser. The reuse branch also verifies `slot._healthy` to guard against a narrow race where `shutdown()` closes a slot between semaphore acquire and the `_started` re-check. After the `_healthy` check, `acquire()` calls `browser.is_connected()` as a final liveness check — if the Chromium process died (OOM kill, crash) between `release()` and the next `acquire()`, the dead slot is closed and a fresh browser is created inline. Both `prune_dead_slots()` **and the rotation branch in `acquire()`** collect/remove dead slots under `_all_locals_lock` but close them **outside** the lock — `_close_slot()` does blocking I/O (Chromium IPC) that could hang, and holding the lock during that call would block all `acquire()` and `shutdown()` callers. `shutdown()` and `prune_dead_slots()` close slots in **daemon threads with a 10s timeout** so the calling thread is never blocked indefinitely. The three `acquire()` close sites (rotation, post-`_started` reuse fail, `_healthy=False` health check) go through `_close_slot_safe()` instead of `_close_slot()` — daemon-threaded + 10s timeout for `browser.close()` and `camoufox_cm.__exit__()`, but `pw.stop()` remains synchronous on the owning thread so the thread-local greenlet/asyncio loop is torn down before the next `_create_browser()` on that thread (otherwise the next start hits "Sync API inside the asyncio loop"; see Bug #24 and the cross-reference with Bugs #6/#22). `close_orphaned_slots()` finds slots whose `_owner_tid` refers to a dead thread (via `threading.enumerate()`) and closes them — called after `pw_executor.shutdown()` to reclaim browsers from dead executor threads. The rotation branch in `acquire()` closes synchronously on the rotating worker thread (intentional — the semaphore is still held, so a daemon thread would leak the slot). The same daemon-thread timeout pattern is used for non-pool (fallback) Playwright cleanup in `close()`. **`_create_browser()` is invoked outside `_all_locals_lock`** (caller `acquire()` releases the lock before the call) and now uses `_safe_sync_playwright_start()` for the Chromium path. On any failure after `sync_playwright().start()` (e.g. `chromium.launch` raising `Executable doesn't exist`), the except branch calls `pw.stop()` / `camoufox_cm.__exit__(...)` on the same creator thread to tear down the half-started runtime — without this, the leaked Playwright sync instance keeps a greenlet-based asyncio loop alive on the worker thread and poisons every subsequent `sync_playwright().start()` on that thread with "Sync API inside the asyncio loop". The cleanup runs on the worker thread (correct thread affinity for Playwright sync) and outside any pool lock (no deadlock risk).
- **`_safe_sync_playwright_start()`** — top-level helper used by both the pool and the non-pool fallback path to start a sync Playwright runtime. On the specific "Sync API inside the asyncio loop" failure it resets the **current thread's** default asyncio loop (`asyncio.get_event_loop().stop/close` then `asyncio.set_event_loop(asyncio.new_event_loop())`) and retries `sync_playwright().start()` once. Touches only the calling thread's loop — no shared state, no locks. Mirrors the long-standing workaround in `niche_scraper._launch_chrome` (niche_scraper.py:584-602). Safe to call from any worker thread.
