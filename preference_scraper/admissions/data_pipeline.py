...
Cleans, normalizes, deduplicates, and exports training data.

Usage:
    python -m preference_scraper.admissions.data_pipeline stats
    python -m preference_scraper.admissions.data_pipeline export
    python -m preference_scraper.admissions.data_pipeline export --format csv
"""
...
from preference_scraper.admissions.db import get_session, init_db, ENGINE
from preference_scraper.admissions.models import School, ApplicantDatapoint
from preference_scraper.admissions.concordance import act_to_sat, sat_to_act
from preference_scraper.admissions.feature_utils import compute_features_df
...
def load_raw_data() -> pd.DataFrame:
    """Load all applicant datapoints joined with school features."""
    query = """
    SELECT
        a.id,
        a.school_id,
...
def deduplicate(df: pd.DataFrame) -> pd.DataFrame:
    """Remove likely duplicate records across sources.

    Match by (school_id, gpa ± 0.05, sat_score ± 20, same outcome).
    Keep the record with more metadata.
    """
    df = df.copy()

    # Round for grouping
    df["_gpa_round"] = (df["gpa"] * 20).round() / 20  # round to 0.05
    df["_sat_round"] = (df["sat_score"] / 20).round() * 20  # round to 20

    # Count non-null metadata fields per row
    meta_cols = ["decision_type", "applicant_type", "gender", "hs_state", "hs_type", "residency"]
    df["_meta_count"] = df[meta_cols].notna().sum(axis=1)

    # Sort by metadata richness (descending) so we keep the richest record
    df = df.sort_values("_meta_count", ascending=False)

    # Drop duplicates on the rounded key, keeping first (most metadata)
    before = len(df)
    df = df.drop_duplicates(
        subset=["school_id", "_gpa_round", "_sat_round", "outcome"],
        keep="first",
    )
    dropped = before - len(df)
    if dropped:
        logger.info(f"Deduplicated: removed {dropped} likely duplicates")

    # Clean up temp columns
    df = df.drop(columns=["_gpa_round", "_sat_round", "_meta_count"])
