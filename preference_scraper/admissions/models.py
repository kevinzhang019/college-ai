...
class ApplicantDatapoint(Base):
    ...
    source = Column(Text, nullable=False)  # 'collegedata' | 'niche'
    gpa = Column(Float)
    sat_score = Column(Float)
    act_score = Column(Float)
    outcome = Column(Text, nullable=False)  # 'accepted' | 'rejected' | 'waitlisted'
    decision_type = Column(Text)    # 'regular' | 'early'
    applicant_type = Column(Text)   # 'standard' | 'legacy' | 'athlete'
    gender = Column(Text)           # 'M' | 'F'
    hs_state = Column(Text)
    hs_type = Column(Text)          # 'public' | 'private' | 'parochial' | 'homeschool'
...
class ScrapeJob(Base):
    ...
    school_slug = Column(Text, nullable=False)
    status = Column(Text, nullable=False, default="pending")  # pending | done | failed
    last_attempt = Column(Text)
    error = Column(Text)

    __table_args__ = (
        UniqueConstraint("source", "school_slug", name="uq_source_slug"),
    )
